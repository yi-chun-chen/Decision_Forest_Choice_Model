...
# CS: bounded consideration set 

function tcm_estimateRanking_CS(N, assortments, transaction_counts, time_limit_overall, perTol, CS_size)
    ...
    @constraint(m_sub, sum(x[i,N] for i in 1:(N-1)) <= CS_size)
    ...
    #@show loglik
    println("CG (CS_size = ", CS_size, ") iter ", iter, " -- time elapsed = ", 0.0, " log likelihood = ",loglik)
	...
	while (time() - start_time < time_limit_overall)
        ...
        println("CG (CS_size = ", CS_size, ") iter ", iter, " -- time elapsed = ", time() - start_time, " log likelihood = ",new_loglik)
