...
# Some of the results generated by this script are also used in Section EC.4.1 (which compares ranking-warm-started heuristic column generation
# and cold-started heuristic column generation) and Section EC.4.4 (which provides runtime and model size results for the ranking, decision forest and LC-MNL models). 

# This script is configured to be run on Amazon EC2 instances. It is accompanied by two other scripts: a helper_fn (helper function) script, IRI_ass_kfold_helperfn.jl,
# and an include script, IRI_ass_kfold_include.jl. 
#
...
# 
# The present script, IRI_ass_kfold.jl, is effectively a custom distributed job scheduler. It creates a list of "jobs" (different instances; a combination of a data set and an 
...
#
# julia IRI_ass_kfold.jl
#
# in the command line. On Amazon EC2, a more useful way to run it is 
#
# nohup julia IRI_ass_kfold.jl & 
#  
# The results will be placed in a folder under tcm_testing/IRI_ass_kfold_test/ . The out_*.csv result files contain the performance of the different methods.
...
#
# A few other important comments:
...
# 	with IRI.
#
# 	- This script only generates part of the results for Section 6.2 of the paper. The rest of the results are generated by the following scripts:
#		* R2_IRI_assRCS_kfold.jl -- this script estimates the ranking-based model with a constrained consideration set, and tunes the consideration set size using 
#									cross-validation.
#		* R2_IRI_assF_RTS_kfold.jl -- this script esimates the decision forest model using the randomized tree sampling method. 
#
#	- In IRI_ass_kfold_helperfn.jl, some code blocks will execute functions for cross-validation (e.g., tcm_estimateMMNL_kcv_weighted_ass, which determines the optimal
#	cross-validated number of classes for the LC-MNL model). These blocks will write the optimal cross-validated parameters to separate files. For completeness, the script
#   also contains blocks that estimate each model with each candidate hyperparameter (e.g., LC-MNL with K = 2,3,4,5,10,15) and computes the out-of-sample performance for the
# 	given test fold. To obtain the cross-validated performance of the model (e.g., "what is the average out-of-sample performance of LC-MNL with K tuned by cross-validation?"),
# 	one can simply join the CSV containing the cross-validated parameter with the CSV containing the complete results for every value of the hyperparameter.
# 
...
include("IRI_ass_kfold_include.jl")

machine = 2;
expdirpath = "../tcm_testing/IRI_ass_kfold_test/"
if (!isdir(expdirpath))
	mkdir(expdirpath);
end
...
initial_helper_script = string( pwd(), "/IRI_ass_kfold_helperfn.jl")
...
while ( !isempty(todo) )
	...
	for i in 1:par_size
		...
		fetch(@spawnat newproc include("IRI_ass_kfold_include.jl"))
