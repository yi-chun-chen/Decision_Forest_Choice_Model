...
# This particular script estimates the decision forest model using the randomized tree sampling method (see Section 5.2).
# Some of the results generated by this script are also used in Section EC.4.3. 
#
...
# and an include script, R2_IRI_assF_RTS_kfold_include.jl. 
#
...
# 
# The present script, R2_IRI_assF_RTS_kfold.jl, is effectively a custom distributed job scheduler. It creates a list of "jobs" (different instances; a combination of a data set and an 
...
#
# julia R2_IRI_assF_RTS_kfold.jl
#
# in the command line. On Amazon EC2, a more useful way to run it is 
#
# nohup julia R2_IRI_assF_RTS_kfold.jl & 
#  
# The results will be placed in a folder under tcm_testing/R2_IRI_assF_RTS_kfold_test/ . The out_*.csv result files contain the performance of the different methods.
...
include("R2_IRI_assF_RTS_kfold_include.jl")

machine = 2;
#expdirpath = mdpart_createPath(machine, "exp_dir", "OptionPricing_custompar_test"); 
expdirpath = "../tcm_testing/R2_IRI_assF_RTS_kfold_test/"
if (!isdir(expdirpath))
	mkdir(expdirpath);
end
...
initial_helper_script = string( pwd(), "/R2_IRI_assF_RTS_kfold_helperfn.jl")
...
# datanames = ["diapers", "sugarsub", "toitisu"]
...
while ( !isempty(todo) )
	...
	for i in 1:par_size
		...
		fetch(@spawnat newproc include("R2_IRI_assF_RTS_kfold_include.jl"))
