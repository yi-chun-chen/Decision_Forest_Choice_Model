# Script to generate the ranking model results for the temporal splitting experiments from Chen and Mišić (see Section EC.5 of the paper) with IRI data.
#
# This script is configured to be run on Amazon EC2 instances. It is accompanied by two other scripts: a helperfn (helper function) script, R2_IRI_temporalRCS_helperfn.jl,
# and an include script, R2_IRI_temporalRCS_include.jl. 
#
...
# 
# The present script, R2_IRI_temporalRCS.jl, is effectively a custom distributed job scheduler. It creates a list of "jobs" (different instances; for this temporal splitting experiment,
...
#
# julia R2_IRI_temporalRCS.jl
#
# in the command line. On Amazon EC2, a more useful way to run it is 
#
# nohup julia R2_IRI_temporalRCS.jl & 
#  
# The results will be placed in a folder under tcm_testing/R2_IRI_temporalRCS_test/ . The out_*.csv result files contain the performance of the different methods.
#
...
include("R2_IRI_temporalRCS_include.jl")

machine = 2;
expdirpath = "../tcm_testing/R2_IRI_temporalRCS_test/"
if (!isdir(expdirpath))
	mkdir(expdirpath);
end
...
initial_helper_script = string( pwd(), "/R2_IRI_temporalRCS_helperfn.jl")
...
while ( !isempty(todo) )
	...
	for i in 1:par_size
		...
		fetch(@spawnat newproc include("R2_IRI_temporalRCS_include.jl"))
