# Script to generate results for the exact column generation experiment from Chen and Mišić (see Section EC.2.2 of the paper) using synthetic data.
#
# This script is configured to be run on Amazon EC2 instances. It is accompanied by two other scripts: a helperfn (helper function) script, synthetic_ECG_helperfn.jl,
# and an include script, synthetic_ECG_include.jl. 
#
...
# 
# The present script, synthetic_ECG.jl, is effectively a custom distributed job scheduler. It creates a list of "jobs" (different instances; for this script, an
# instance is a value of N, M, d and a method -- one of exact column generation, heuristic column generation or randomized tree sampling). 
...
#
# julia synthetic_ECG.jl
#
# in the command line. On Amazon EC2, a more useful way to run it is 
#
# nohup julia synthetic_ECG.jl & 
#  
# The results will be placed in a folder under tcm_testing/synthetic_ECG_test/ . The out_*.csv result files contain the performance of the different methods.
#
...
include("synthetic_ECG_include.jl")


machine = 2;
expdirpath = "../tcm_testing/synthetic_ECG_test/"
if (!isdir(expdirpath))
	mkdir(expdirpath);
end
...
initial_helper_script = string( pwd(), "/synthetic_ECG_helperfn.jl")
...
method_vec = ["ECG", "HCG", "RTS"]
N_vec = [5,7,9]
M_vec = [10, 20, 50, 100, 200]
d_vec = [3,4,5]
	...
	for N in N_vec
		for d in d_vec
			...
			for M in M_vec
				...
				push!(instances_vec, [method, N, M, d] )
			end
...
while ( !isempty(todo) )
	...
	for i in 1:par_size
		...
		fetch(@spawnat newproc include("synthetic_ECG_include.jl"))
